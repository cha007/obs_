...
struct ScopedLockBase{
	bool locked, unlock;
	HANDLE h;

	ScopedLockBase(HANDLE mutex, bool tryLock = false, bool autounlock = true) : locked(false), unlock(autounlock), h(mutex)
	{
		if (!h) return;
		if (tryLock && !OSTryEnterMutex(h)) return;
		if (!tryLock) OSEnterMutex(h);
...
struct ScopedLock : public ScopedLockBase
{
	ScopedLock(std::unique_ptr<void, MutexDeleter> const &mutex, bool tryLock = false, bool autounlock = true) :ScopedLockBase(mutex.get(), tryLock, autounlock){
	}
};
template <class Fun>
struct ScopeGuard
