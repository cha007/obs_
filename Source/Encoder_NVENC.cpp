...
bool _CheckNVENCHardwareSupport(bool log=true){
	return CheckNVENCHardwareSupport(log);
}
...
{
	if (!_CheckNVENCHardwareSupport(true)){
		return NULL;
	}

	return CreateNVENCEncoder(fps, width, height, quality, preset, bUse444, colorDesc, maxBitRate, bufferSize, bUseCFR);
}
