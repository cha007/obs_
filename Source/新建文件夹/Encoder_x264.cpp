...
public:
    ...
    }*/
	{
			...
			curProfile = TEXT("main");

			BOOL bUseCustomParams = false;
			zero(&paramData, sizeof(paramData));

			char lpPreset[20] = "veryfast";

			if (x264_param_default_preset(&paramData, lpPreset, NULL))
				Log(TEXT("Failed to set x264 defaults: %s/%s"), curPreset.Array(), curTune.Array());
			...
			bUseCBR = true;//	AppConfig->GetInt(TEXT("Video Encoding"), TEXT("UseCBR"), 1) != 0;
			bPadCBR = true;//	AppConfig->GetInt(TEXT("Video Encoding"), TEXT("PadCBR"), 1) != 0;
			this->bUseCFR = bUseCFR;
			...
			UINT keyframeInterval = 0;// AppConfig->GetInt(TEXT("Video Encoding"), TEXT("KeyframeInterval"), 0);
